...
if [[ "$CENTOS_NINE" -eq '9' ]]; then
  PHPVERLATEST=$(curl -${ipv_forceopt}sL https://www.php.net/downloads.php| grep -E -o "php-[0-9.]+\.tar[.a-z]*" | grep -v '.asc' | awk -F "php-" '/.tar.gz$/ {print $2}' | sed -e 's|.tar.gz||g' | uniq | grep '8.2' | head -n1)
elif [[ "$CENTOS_EIGHT" -eq '8' ]]; then
  PHPVERLATEST=$(curl -${ipv_forceopt}sL https://www.php.net/downloads.php| grep -E -o "php-[0-9.]+\.tar[.a-z]*" | grep -v '.asc' | awk -F "php-" '/.tar.gz$/ {print $2}' | sed -e 's|.tar.gz||g' | uniq | grep '8.2' | head -n1)
else
  PHPVERLATEST=$(curl -${ipv_forceopt}sL https://www.php.net/downloads.php| grep -E -o "php-[0-9.]+\.tar[.a-z]*" | grep -v '.asc' | awk -F "php-" '/.tar.gz$/ {print $2}' | sed -e 's|.tar.gz||g' | uniq | grep '8.2' | head -n1)
