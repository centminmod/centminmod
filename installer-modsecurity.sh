...
if [[ "$CENTOS_NINE" -eq '9' ]]; then
  ...
  echo "NGINX_MODSECURITY='y'" >> /etc/centminmod/custom_config.inc
fi
# set el8 defaults
...
if [[ "$CENTOS_NINE" -eq '9' ]]; then
  ...
  PHPVERLATEST=$(curl -${ipv_forceopt}sL https://www.php.net/downloads.php?source=Y| egrep -o "php\-[0-9.]+\.tar[.a-z]*" | grep -v '.asc' | awk -F "php-" '/.tar.gz$/ {print $2}' | sed -e 's|.tar.gz||g' | uniq | grep '8.0' | head -n1)
else
