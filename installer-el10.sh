...
if [[ ! -f /.dockerenv ]]; then
  ...
  if rpm -q coreutils-single >/dev/null 2>&1; then
    ...
    yum -y install coreutils --allowerasing
...
lowmemcheck() {
  ...
  if [ "$CHECKMINMEM" == "1" ]; then
    if [ "$TOTALMEM_SWAP" -lt "$ISMINSWAP" ]; then
      ...
      echo ""
      if [[ "$CENTOS_TEN" -eq '10' ]]; then
        echo "For EL10 operating system the minimum and recommended memory requirements have increased"
...
if [[ ! -f /.dockerenv && "$CHECK_LXD" != 'y' ]]; then
  ...
  if [[ "$CENTOS_TEN" -eq '10' ]] && [[ -z "$SELINUX_STATUS_GRUB" ]]; then
...
# set el10 to utf8mb4 charset for MariaDB 10.11
if [[ "$CENTOS_TEN" -eq '10' ]]; then
...
if [[ "$CENTOS_ALPHATEST" != [yY] && "$CENTOS_TEN" -eq '10' ]] || [[ "$CENTOS_ALPHATEST" != [yY] && "$CENTOS_EIGHT" -eq '8' ]] || [[ "$CENTOS_ALPHATEST" != [yY] && "$CENTOS_NINE" -eq '9' ]]; then
...
if [[ "$CENTOS_SEVEN" -eq '7' || "$CENTOS_EIGHT" -eq '8' || "$CENTOS_NINE" -eq '9' || "$CENTOS_TEN" -eq '10' ]] && [ ! -f /etc/rc.d/rc.local ]; then
...
if [[ "$CENTOS_TEN" -eq '10' ]]; then
  #########################################################
  # EL10 CURL INSTALLER BATCH OPTIMIZATION
  # Organized package installation for better progress tracking
  # Matches git clone path batch optimization structure
  # Total packages: ~200 split into 3 phases
  #########################################################

  echo "*************************************************"
  echo "* EL10 Curl Installer: Batch Package Installation"
  echo "* Phase 1/3: Core Development Tools (~95 packages)"
  echo "*************************************************"
  CURL_BATCH1_START=$(date +%s)

  # Group 1: Core Development Tools
  # Compilers, system tools, perl modules, network utilities
  time $YUMDNFBIN -y install \
    virt-what gawk unzip bc wget lynx screen ca-certificates yum-utils bash plocate \
    subversion rsyslog dos2unix boost-program-options net-tools imake bind-utils \
    libatomic_ops-devel time coreutils autoconf cronie crontabs cronie-anacron gcc gcc-c++ \
    automake libtool make libXext-devel patch sysstat openssh flex bison file \
    libtool-ltdl-devel krb5-devel libXpm-devel nano gmp-devel aspell-devel numactl lsof \
    pkgconfig gdbm-devel tk-devel bluez-libs-devel iptables-nft iptables-nft-services \
    iptables-libs iptables-utils rrdtool diffutils which perl-Math-BigInt perl-Test-Simple \
    perl-ExtUtils-Embed perl-ExtUtils-MakeMaker perl-Time-HiRes perl-libwww-perl \
    perl-Net-SSLeay cyrus-imapd cyrus-sasl-md5 cyrus-sasl-plain strace cmake git \
    net-snmp-libs net-snmp-utils iotop perl-FindBin perl-diagnostics systemd-devel \
  ...
  echo "*************************************************"
  echo "* Phase 1 completed in ${CURL_BATCH1_DURATION} seconds"
  echo "*************************************************"

  echo "*************************************************"
  echo "* Phase 2/3: Development Library Pairs (~60 packages)"
  echo "*************************************************"
  CURL_BATCH2_START=$(date +%s)

  # Group 2: Development Library Pairs
  # Base libraries + corresponding -devel packages
  time $YUMDNFBIN -y install \
    libvpx libvpx-devel t1lib t1lib-devel expect readline readline-devel \
    libedit libedit-devel libxslt libxslt-devel openssl openssl-devel curl curl-devel \
    openldap openldap-devel zlib zlib-devel gd gd-devel pcre pcre-devel \
    gettext gettext-devel libidn libidn-devel libjpeg libjpeg-devel libpng libpng-devel \
    freetype freetype-devel libxml2 libxml2-devel glib2 glib2-devel bzip2 bzip2-devel \
    ncurses ncurses-devel e2fsprogs e2fsprogs-devel cyrus-sasl cyrus-sasl-devel \
  ...
  echo "*************************************************"
  echo "* Phase 2 completed in ${CURL_BATCH2_DURATION} seconds"
  echo "*************************************************"

  echo "*************************************************"
  echo "* Phase 3/3: Graphics and X11 Libraries (~45 packages)"
  echo "*************************************************"
  CURL_BATCH3_START=$(date +%s)

  # Group 3: Graphics, X11, and Image Processing Libraries
  time $YUMDNFBIN -y install \
    OpenEXR-devel OpenEXR-libs atk cups-libs fftw-libs-double fribidi gdk-pixbuf2 \
    ghostscript-devel gl-manpages graphviz gtk2 hicolor-icon-theme ilmbase ilmbase-devel \
    jasper-devel jasper-libs jbigkit-devel jbigkit-libs lcms2 lcms2-devel libICE-devel \
    libSM-devel libXaw libXcomposite libXcursor libXdamage-devel libXfixes-devel libXi \
  ...
  echo "*************************************************"
  echo "* Phase 3 completed in ${CURL_BATCH3_DURATION} seconds"
  echo "* Total Phase 1-3 time: $((CURL_BATCH1_DURATION + CURL_BATCH2_DURATION + CURL_BATCH3_DURATION)) seconds"
  echo "*************************************************"
elif [[ "$CENTOS_NINE" -eq '9' ]]; then
  ...
  if [[ "$CENTOS_TEN" = '10' ]]; then
    #########################################################
    # EL10 CURL INSTALLER: Phase 4 - Utility and Compression Tools
    # Split into standard packages and REMI repository packages
    #########################################################

    echo "*************************************************"
    echo "* Phase 4/5: Utility and Compression Tools"
    echo "*************************************************"
    ...
    time $YUMDNFBIN -y install \
      ...
      bash-completion mlocate re2c ${KERNEL_PKGS}${DISABLEREPO_DNF} \
...
if [[ "$CENTOS_TEN" -eq '10' ]]; then
  PHPVERLATEST=$(curl -${ipv_forceopt}sL https://www.php.net/downloads.php?source=Y| grep -E -o "php-[0-9.]+\.tar[.a-z]*" | grep -v '.asc' | awk -F "php-" '/.tar.gz$/ {print $2}' | sed -e 's|.tar.gz||g' | uniq | grep '8.3' | head -n1)
elif [[ "$CENTOS_NINE" -eq '9' ]]; then
  PHPVERLATEST=$(curl -${ipv_forceopt}sL https://www.php.net/downloads.php?source=Y| grep -E -o "php-[0-9.]+\.tar[.a-z]*" | grep -v '.asc' | awk -F "php-" '/.tar.gz$/ {print $2}' | sed -e 's|.tar.gz||g' | uniq | grep '8.3' | head -n1)
elif [[ "$CENTOS_EIGHT" -eq '8' ]]; then
  PHPVERLATEST=$(curl -${ipv_forceopt}sL https://www.php.net/downloads.php?source=Y| grep -E -o "php-[0-9.]+\.tar[.a-z]*" | grep -v '.asc' | awk -F "php-" '/.tar.gz$/ {print $2}' | sed -e 's|.tar.gz||g' | uniq | grep '8.3' | head -n1)
...
if [[ "$CENTOS_TEN" -eq '10' ]]; then
  PHPVERLATEST=${PHPVERLATEST:-"8.3.26"}
elif [[ "$CENTOS_NINE" -eq '9' ]]; then
  PHPVERLATEST=${PHPVERLATEST:-"8.3.26"}
elif [[ "$CENTOS_EIGHT" -eq '8' ]]; then
  PHPVERLATEST=${PHPVERLATEST:-"8.3.26"}
