...
if [[ "$CENTOS_NINE" -eq '9' ]]; then
  ...
  echo "NGINX_DYNAMICTLS='y'" >> /etc/centminmod/custom_config.inc
  echo "NGINX_PCRE_TWO='y'" >> /etc/centminmod/custom_config.inc
  echo "OPENSSL_VERSION='3.1.4'" >> /etc/centminmod/custom_config.inc
  echo "OPENSSL_VERSIONFALLBACK='3.1.4'" >> /etc/centminmod/custom_config.inc
  echo "OPENSSL_VERSION_OLDOVERRIDE='3.1.4'" >> /etc/centminmod/custom_config.inc
  echo "NGINX_NJS='y'" >> /etc/centminmod/custom_config.inc
  echo "NGXDYNAMIC_NJS='y'" >> /etc/centminmod/custom_config.inc
...
if [[ "$CENTOS_NINE" -eq '9' ]]; then
  PHPVERLATEST=$(curl -${ipv_forceopt}sL https://www.php.net/downloads.php?source=Y| egrep -o "php\-[0-9.]+\.tar[.a-z]*" | grep -v '.asc' | awk -F "php-" '/.tar.gz$/ {print $2}' | sed -e 's|.tar.gz||g' | uniq | grep '8.1' | head -n1)
elif [[ "$CENTOS_EIGHT" -eq '8' ]]; then
  PHPVERLATEST=$(curl -${ipv_forceopt}sL https://www.php.net/downloads.php?source=Y| egrep -o "php\-[0-9.]+\.tar[.a-z]*" | grep -v '.asc' | awk -F "php-" '/.tar.gz$/ {print $2}' | sed -e 's|.tar.gz||g' | uniq | grep '8.1' | head -n1)
else
  PHPVERLATEST=$(curl -${ipv_forceopt}sL https://www.php.net/downloads.php?source=Y| egrep -o "php\-[0-9.]+\.tar[.a-z]*" | grep -v '.asc' | awk -F "php-" '/.tar.gz$/ {print $2}' | sed -e 's|.tar.gz||g' | uniq | grep '8.1' | head -n1)
...
if [[ "$CENTOS_NINE" -eq '9' ]]; then
  PHPVERLATEST=${PHPVERLATEST:-"8.2.32"}
elif [[ "$CENTOS_EIGHT" -eq '8' ]]; then
  PHPVERLATEST=${PHPVERLATEST:-"8.2.32"}
else
  PHPVERLATEST=${PHPVERLATEST:-"8.2.32"}
